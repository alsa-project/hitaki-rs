...
impl<O: IsA<MotuRegisterDsp>> MotuRegisterDspExtManual for O {
    fn read_parameter(&self, param: &mut SndMotuRegisterDspParameter) -> Result<(), glib::Error> {
        unsafe {
            ...
            hitaki_sys::hitaki_motu_register_dsp_read_parameter(
    ...
    fn read_byte_meter(&self, meter: &mut [u8; 48]) -> Result<(), glib::Error> {
        unsafe {
            ...
            hitaki_sys::hitaki_motu_register_dsp_read_byte_meter(
                self.as_ref().to_glib_none().0,
                &ptr,
                &mut error,
            );
        ...
        F: Fn(&Self, &[u32]) + 'static,
    {
        unsafe extern "C" fn changed_trampoline<P, F>(
            this: *mut hitaki_sys::HitakiMotuRegisterDsp,
            events: *const u32,
            length: libc::c_uint,
            f: glib_sys::gpointer,
        ) where
    ...
    fn emit_changed(&self, events: &[u32]) {
        unsafe {
            ...
            gobject_sys::g_value_set_uint(args[2].to_glib_none_mut().0, events.len() as u32);
